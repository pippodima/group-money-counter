...
| D60 | Screenshots are generated by driving the real UI, not curated | A hand-taken screenshot silently goes stale; a script that clicks through the actual flows breaks when the flow breaks | 14 |
...
## Entry 14 — 2026-09-03 · Making the repo legible to someone else

The repo had no README, no licence and no description. Nothing told a visitor what it was
or whether they were allowed to use it.

Added a README, MIT licence, and the GitHub-side metadata — description, homepage, topics.

### What the README leads with

Not a feature list. The constraint: with no server to arbitrate, *"who has the right
numbers?"* has to be answered by the data structure instead. That is the only genuinely
interesting thing here, and the union-merge explanation carries the rest.

The privacy claim is stated with its limits attached — the app makes no network requests and
CI enforces it, **and** you can export a file and send it, and `navigator.share` hands it to
the OS. Overclaiming there is the one thing that would make everything else in the document
untrustworthy.

### Screenshots are generated

`npm run screenshots` drives Chromium through the real interface — creates a group, adds five
expenses, opens the split editor, walks the tabs — and captures what comes out (**D60**).

Hand-taken screenshots go stale silently: the UI moves on and the pictures keep advertising
an older app. A script that clicks through the actual flows cannot, because it breaks when
the flow breaks. It found the stale copy on the landing screen immediately — still promising
"there is no link to send" after remote invites had shipped.

Two things it taught me about Playwright:

- `storageState` carries cookies and localStorage but **not IndexedDB**, so the dark-mode
  shot opened a fresh context onto an empty app. `page.emulateMedia({ colorScheme: 'dark' })`
  on the existing page keeps the data.
- A native `<input type="date">` formats from the *browser's* UI language, not the page
  locale, so `--lang=en-GB` at launch was needed on top of the context's `locale`.

Each run mints a new group id, so the whole set takes whichever colour that id hashes to.
Consistent within a run, different between them — which is a fair advertisement for the
feature.
